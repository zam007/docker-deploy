...
docker pull harbor.tiejin.cn/develop/smartlog_server:${dockerTag}
...
docker run --name=smartlog_server -d -p 9821:9821 -e UMSCLOUD_FILESERVER_OPTS="-Denv=test \
   -server -Xmx3600m -Xss512k -Dfile.encoding=utf-8 -XX:MaxMetaspaceSize=100m\
    ...
    -v /root/.aws:/root/.aws \
    -v /logs/ums_fileserver_test_0/:/logs/server/ harbor.tiejin.cn/develop/smartlog_server:latest
