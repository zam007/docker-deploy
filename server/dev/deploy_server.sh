...
cp $(cd `dirname $0`; pwd)/../cluster/cluster_dev.json /data/umscloud-server/conf/cluster.json
...
docker run --name=ums_scheduler_sandbox -d -p 8085:8080 -e UMSCLOUD_SCHEDULER_OPTS="-Denv=sandbox \
 -server -Xmx3600m -Xss512k -Dfile.encoding=utf-8 -XX:MaxMetaspaceSize=100m\
    ...
    -v /logs/ums_scheduler_sandbox/:/logs/server/ harbor.tiejin.cn/develop/umscloud-scheduler:${dockerTag} \
